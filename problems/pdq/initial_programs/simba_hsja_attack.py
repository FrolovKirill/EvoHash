"""SimBa (block basis) + HSJA hybrid attack for PDQ.
...
Phase 2 — SimBa block-basis refinement: reduces L2 distortion using
coordinate descent along pixel-block basis vectors.
...
def simba_block_refine_single(source, start, target_hash, hash_fn, threshold,
                              n_iter=120, step_size=12.0, block_size=16):
    H, W, _ = source.shape
    current = start.copy()
...
def _attack_single(src_img, tgt_img, target_hash, hash_fn, threshold,
                   ...
                   refine_n_iter=120, refine_step_size=12.0, refine_block_size=16):
    ...
    # Phase 2: SimBa block refinement
    best, best_dist, q2 = simba_block_refine_single(
        source, hsja_result, target_hash, hash_fn, threshold,
        n_iter=refine_n_iter, step_size=refine_step_size, block_size=refine_block_size,
    )
