...
Phase 1: ZO-SignSGD finds collision fast (no L2 constraint).
Phase 2: HSJA minimizes L2 distortion while maintaining collision.

Reference: Liu et al., ICLR 2019 (ZO-SignSGD); Chen et al., IEEE S&P 2020 (HSJA).
"""
...
def _zo_phase1(img, target_hash, hash_fn, threshold,
               n_iter=500, n_samples=20, mu=80.0, lr=8.0):
...
def _attack_single(src_img, target_hash, hash_fn, threshold,
                   p1_n_iter=500, p1_n_samples=20, p1_mu=80.0, p1_lr=8.0,
                   hsja_n_iter=25, hsja_bs_steps=10, hsja_grad_samples=20):
    source = np.array(src_img).astype(np.float32)

    # Phase 1: ZO-SignSGD finds collision
    phase1_img, phase1_metrics = _zo_phase1(
        src_img, target_hash, hash_fn, threshold,
        n_iter=p1_n_iter, n_samples=p1_n_samples, mu=p1_mu, lr=p1_lr,
    )
